...
English blog draft and a Beamer carousel with 9 frames and 12 PDF pages, including a full-graph introduction, an uncertainty reveal and a three-step surplus reveal. Parameters are invented; there is no empirical calibration or recommended time quota. The model quantifies weighted research progress, not creativity or completed papers.
...
- `learning-detours.tex`: the carousel. TikZ illustrations plus vector graphs exported from the blog's plotting code; numbers come from `model-values.tex`. Explicit Beamer `\only` overlays provide pause-style reveals with fixed chart positions; PDF page numbers count each reveal.
- `ct-lt-carousel-intro.pdf` and `ct-lt-carousel-basic.pdf`: generated by `simulate.R` from the same curves, crossings and palette as the blog graphs, with larger text and stacked legends. The first view shows the full ten-hour budget with certain gains and arrows explaining both curves and axes. The second adds uncertainty and zooms to the first 3.5 hours. The quality extension is excluded from the carousel; model parameters are unchanged.
- `ct-lt-carousel-surplus-1.pdf` through `ct-lt-carousel-surplus-3.pdf`: fixed-axis zooms revealing pessimistic odds, true odds, then the shaded risk-adjusted surplus. The sequence distinguishes extra exploration time from the surplus's equivalent value in current research time.
